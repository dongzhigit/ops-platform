...
class SSH:
    def __init__(self, hostname, port=22, username='root', pkey=None, password=None, default_env=None,
        ...
        self.exec_file = None
        self.term = term or {}
        self.eof = 'ops-platform EOF 2108111926'
        self.default_env = default_env
        self.regex = re.compile(r'ops-platform EOF 2108111926 (-?\d+)[\r\n]?')
        self.arguments = {
            'hostname': hostname,
            'port': port,
            'username': username,
    ...
    def replace_file(self, source_path, destination_path, overwrite=True):
        ...
        backup_path = posixpath.join(
            ...
            '.ops-platform-backup-%s' % uuid4().hex,
        )
    ...
    def _get_channel(self):
        ...
        while True:
            if self.channel.recv_ready():
                ...
                self.client.close()
                raise Exception('Wait ops-platform response timeout')
            else:
                counter += 1
                time.sleep(0.1)
        return self.channel
    ...
    def _handle_command(self, command, environment):
        ...
        if not self.exec_file:
            self.exec_file = f'/tmp/ops-platform.{uuid4().hex}'
