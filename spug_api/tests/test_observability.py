...
class ObservabilityTest(TestCase):
    ...
    def alert_payload(self, status='firing', fingerprint='abc123'):
        return {
            ...
            'alerts': [{
                ...
                'labels': {
                    'alertname': 'SpugHostCpuHigh',
                    'severity': 'warning',
                    'instance': '10.20.30.40:9100',
                    'spug_host_id': str(self.host.id),
    ...
    def test_discovery_uses_independent_bearer_and_reserved_host_labels(self):
        ...
        self.assertEqual(data[0]['labels']['spug_host_id'], str(self.host.id))
    ...
    def test_metric_query_is_predefined_and_host_authorized(self, get):
        ...
        self.assertIn('spug_host_id="%s"' % self.host.id, query)
    ...
    def test_claim_and_real_alertmanager_silence_are_audited(self, delete_silence, create_silence):
        ...
        event = AlertEvent.objects.create(
            ...
            host=self.host,
            alert_name='OpsAlert',
            labels=json.dumps({'alertname': 'OpsAlert', 'spug_host_id': str(self.host.id)}),
            annotations='{}',
        )
    ...
    def test_alertmanager_silence_uses_utc_wire_timestamps(self, post):
        ...
        event = AlertEvent.objects.create(
            ...
            host=self.host,
            alert_name='UtcAlert',
            labels=json.dumps({'alertname': 'UtcAlert', 'spug_host_id': str(self.host.id)}),
            annotations='{}',
        )
