...
- React 生产构建包含知识空间、成员、文档版本、发布/恢复、授权搜索、只读 AI 调查的证据预览和结果分区界面，以及支持 OpenAI/Anthropic 格式切换的模型配置弹窗；知识和模型输出均按纯文本渲染。
- 全量 Django 迁移基线可在 MariaDB 空库安装到 `knowledge.0001` 与 `aiops.0003`，模型漂移检查通过。
...
- `linux/amd64` alpha.13 镜像完整构建流程通过，OCI 版本标签和应用版本均为 `0.1.0-alpha.13`，镜像包含当前仓库后端和镜像构建阶段重新编译的前端；发布镜像以最终提交哈希写入 OCI revision。
