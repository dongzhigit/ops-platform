...
- 收集当前用户有权访问的主机基础信息、Prometheus 指标、Alertmanager 告警、已发布知识片段，以及在具备该主机 `ssh.connect` 授权时由后端固定白名单采集的只读运行态服务快照。
...
- 大模型不可访问 SSH、Shell、数据库、凭据、发布、配置或审批工具；运行态服务快照由 ops-platform 后端执行固定只读命令后作为证据传入模型。

## 强制安全边界

资产描述、告警、指标、运行态快照和知识内容全部标记为不可信输入。系统提示明确要求忽略其中的指令，且模型请求不携带 `tools` 或 `functions`。后端在保存结果前再次递归拒绝 `command`、`shell`、`script`、`tool_call`、`arguments` 等可执行字段，并校验每个引用都属于本次证据集合。

运行态服务快照只在当前用户同时具备主机查看权限和该主机 `ssh.connect` 授权时采集。后端仅执行内置固定查询：运行中的 systemd/legacy service 单元、监听 TCP/UDP 端口、按 CPU 排序的进程名称快照；不接收模型生成命令，不接收用户自定义命令，不采集进程完整命令行参数，避免把命令行密钥或 token 带入模型证据。
...
生产模式下公网模型服务地址必须使用 HTTPS。对于部署在可信隔离网络中的模型网关，回环地址、`host.docker.internal`、`gateway.docker.internal` 和 RFC1918 内网 IP 可使用 HTTP；普通 DNS 主机名不会通过运行时解析结果自动放行，避免 DNS 重绑定绕过。内网 HTTP 不提供传输加密，不应跨越不受信任网络。
...
- 不采集原始日志、不执行自定义命令、不把模型输出转成命令；只有已接入的资产、指标、告警、知识证据和受 `ssh.connect` 授权保护的固定只读运行态快照。
