...
当前进度：SFTP 上传已完成持久化单文件会话和最多 50 个文件的批次队列、4 MiB 浏览器分片、2 路受控文件并发、暂停/恢复/失败清理后重试/取消，以及 24 小时内断点续传、分片与远端整文件 SHA-256 校验和安全覆盖；下载已支持单段 HTTP Range、HEAD、`If-Range`、中断审计和文件列表多选下载入口，Scheduler 每 15 分钟清理过期、失败或取消的上传临时文件。用户可选冲突策略、在线编辑锁、Guacamole、RDP/VNC 和会话录像尚未完成。实现说明见 [M2 SFTP 续传与清理](M2_RESUMABLE_SFTP.md)。
