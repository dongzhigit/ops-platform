...
当前进度：单文件 SFTP 上传已完成持久化会话、4 MiB 浏览器分片、24 小时内断点续传、分片与远端整文件 SHA-256 校验及校验后安全覆盖；下载已支持单段 HTTP Range、HEAD、`If-Range` 和中断审计，Scheduler 每 15 分钟清理过期上传临时文件。批量队列、用户可选冲突策略、在线编辑锁、Guacamole 和会话录像尚未完成。实现说明见 [M2 SFTP 续传与清理](M2_RESUMABLE_SFTP.md)。
