...
当前进度：单文件 SFTP 上传已完成持久化会话、4 MiB 浏览器分片、24 小时内断点续传、分片与远端整文件 SHA-256 校验，以及校验后安全覆盖。断点下载、批量队列、用户可选冲突策略、过期临时文件自动清理、在线编辑锁、Guacamole 和会话录像尚未完成。实现说明见 [M2 SFTP 分片上传与断点续传](M2_RESUMABLE_SFTP.md)。
