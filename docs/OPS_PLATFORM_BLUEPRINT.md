...
当前进度：已完成资产凭据/身份/绑定/对象授权的后端纵向链路，以及 SSH、SFTP、批量、计划、监控和发布的执行前授权；已增加高风险操作双人审批、一次性批准票据、关联 ID 和 HMAC 审计链，并接入批量命令、文件写入/分发、计划任务及原有发布审批事件。审批前端、任务证据、配置中心增强、知识库与只读 AI 仍在开发。实现说明见 [M1 资产、凭据与对象授权](M1_ASSET_ACCESS.md) 和 [M1 高风险操作审批与审计](M1_APPROVAL_AUDIT.md)。
