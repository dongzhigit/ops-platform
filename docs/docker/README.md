...
镜像会直接编译当前仓库的 `ops_api` 和 `ops_web`，不会在启动时下载上游版本。更新代码后必须重新执行 `docker compose up -d --build`。运行日志、临时文件和构建仓库分别持久化，应用源码保留在不可变镜像层中。

首次启动后创建管理员：

```bash
docker exec ops-platform init_ops_platform admin '请替换为高强度密码'
```
...
- 限制 ops-platform 到模型服务的出站地址；模型接口不得回连资产网络，也不得复用应用、数据库或监控密钥。
...
覆盖文件也取消固定容器名，可用 `-p` 在不影响现有实例的情况下启动并行预发布环境，例如在上述命令中加入 `-p ops-platform-preview`。
...
- `docker compose ps` 中数据库、Guacamole 和 ops-platform 服务状态正常。
