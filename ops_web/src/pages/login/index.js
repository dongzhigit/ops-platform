...
import logo from 'layout/ops-platform-logo.svg';
      ...
      .then(data => {
        if (data['required_mfa']) {
          ...
          Modal.warning({
            ...
            content: <div>未能获取到访问者的真实IP，无法提供基于请求来源IP的合法性验证，请检查反向代理真实 IP 透传配置。</div>,
            onOk: () => doLogin(data)
          })
        } else {
          doLogin(data)
  ...
  return (
    <div className={styles.container}>
      <div className={styles.titleContainer}>
        <div><img className={styles.logo} src={logo} alt="ops-platform"/></div>
        <div className={styles.desc}>统一运维、资产访问、监控告警与 AI 调查入口</div>
      </div>
      ...
      <div className={styles.footerZone}>
        <div style={{color: 'rgba(0, 0, 0, .45)'}}>Copyright <CopyrightOutlined/> {new Date().getFullYear()} By ops-platform</div>
      </div>
    </div>
  )
}
