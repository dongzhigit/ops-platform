      ...
      <Form labelCol={{span: 6}} wrapperCol={{span: 14}} className={styles.ext2Form}>
        {store.deploy.id === undefined && (
          <Alert
            ...
            description={[
              <p key={1}>ops-platform 将遵循先本地后目标主机的原则，按照顺序依次执行添加的动作，例如：本地动作1 -> 本地动作2 -> 目标主机动作1 -> 目标主机动作2 ...</p>,
            ...
            {item['type'] === 'transfer' ? ([
              <Form.Item key={0} required label="数据来源">
                <Input
                  ...
                  placeholder="请输入本地（部署 ops-platform 的容器或主机）路径"
              ...
              <Form.Item key={2} required label="目标路径" extra="使用前请确认源路径、目标路径、目标主机权限和传输覆盖策略。">
                <Input
